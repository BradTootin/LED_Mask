...
void randomHueBlink(CHSV *hsvLeft, CHSV *hsvRight, CRGB *rgbLeft, CRGB *rgbRight, int8_t size_array,
                    uint8_t num_pixels, uint8_t num_blinks, bool *litLeft, bool*litRight, int wait) { // TESTED
  for (uint8_t j = 0; j < num_blinks; j++) {
    clear_leds(hsvLeft, hsvRight, rgbLeft, rgbRight, size_array);
    uint8_t numLit = findNumLit(hsvLeft,size_array,litLeft);
    while (numLit < num_pixels) {
      int8_t ind = random8(0,size_array);
